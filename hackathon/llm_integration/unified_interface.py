...
class UnifiedLLMInterface:
    ...
    def is_service_available(self) -> bool:
        """Check if the LLM service is available (alias for is_available for compatibility)."""
        return self.is_available()
    
    def get_model_info(self) -> Dict[str, Any]:
        """Get model information from the current provider."""
        if not self.is_available():
            return {"error": "No LLM provider available"}
        
        if hasattr(self.interface, 'get_model_info'):
    ...
    def generate_insights(self, data, context: Optional[Dict[str, Any]] = None) -> str:
        """
        Generate insights from energy data or text prompt.
        
        Args:
            data: DataFrame containing energy data OR string prompt for analysis
            context: Optional context information
            
        Returns:
            Generated insights about the energy data or prompt
        """
        if not self.is_available():
        ...
        try:
            ...
            if isinstance(data, str):
                # String prompt - pass directly to interface
                insights = self.interface.generate_insights(data, context)
            else:
                # DataFrame - pass to interface (existing behavior)
                insights = self.interface.generate_insights(data, context)
